...
use ntapi::ntapi_base::{OBJECT_ATTRIBUTES, CLIENT_ID};
...
unsafe fn kill_process(pid: usize) -> bool {
    ...
    let mut client_id = CLIENT_ID { UniqueProcess: pid as *mut _, UniqueThread: null_mut() };
    
    // Приводим handle.0 к нужному типу указателя
    let mut handle_ptr = handle.0;
    if ZwOpenProcess(&mut handle_ptr, PROCESS_TERMINATE.0, &mut obj_attr, &mut client_id) == STATUS_SUCCESS.0 {
        handle.0 = handle_ptr;
...
unsafe fn scan_network() {
    ...
    if GetExtendedTcpTable(Some(buffer.as_mut_ptr() as *mut _), &mut table_size, false, AF_INET.0 as u32, TCP_TABLE_OWNER_PID_ALL, 0) == 0 {
...
unsafe fn peek_memory(pid: usize, addr: usize) -> u64 {
    ...
    // Приводим handle.0 к нужному типу указателя
    let mut handle_ptr = handle.0;
    if ZwOpenProcess(&mut handle_ptr, (PROCESS_VM_READ | PROCESS_QUERY_INFORMATION).0, &mut obj_attr, &mut client_id) == STATUS_SUCCESS.0 {
        handle.0 = handle_ptr;
        let mut value = 0u64;
        let mut read = 0;
        ZwReadVirtualMemory(handle.0, addr as *mut _, &mut value as *mut _ as *mut _, 8, &mut read);
        return value;
    }
    0
}
